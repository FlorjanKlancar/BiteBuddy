...
export default function ProgressPage() {
	...
	useEffect(() => {
		api<WeeklySummary[]>("/api/stats/weekly").then(setWeekly).catch(console.error);
		api<{ streak: number }>("/api/stats/streak")
			.then((s) => setStreak(s.streak))
			.catch(console.error);
		api<{ calorieTarget?: number } | null>("/api/profile")
			.then((p) => {
	...
	const consistency = weekly.length > 0 ? Math.round((daysOnTarget / weekly.length) * 100) : 0;
	...
	return (
		<div className="space-y-5">
			...
			<FadeIn delay={0.1}>
				<div className="grid grid-cols-3 gap-2">
					...
					<div className="bg-card rounded-2xl p-4 text-center shadow-[0_12px_32px_rgba(18,28,42,0.04)] border border-outline-variant/10">
						...
						<p className="text-xl font-bold text-foreground">{daysOnTarget}</p>
			...
			<FadeIn delay={0.15}>
				<div className="bg-card rounded-3xl p-5 shadow-[0_12px_32px_rgba(18,28,42,0.04)] border border-outline-variant/10">
					<div className="flex items-center justify-between mb-4">
						<div>
							<p className="text-sm font-bold text-foreground">Weekly Overview</p>
					...
					{weekly.length > 0 ? (
						<div className="flex items-end gap-2 h-36">
							{weekly.map((day) => {
								...
								const isOnTarget =
									day.totalCalories > 0 && day.totalCalories <= calorieTarget * 1.1;
						...
						<div className="h-36 flex items-center justify-center">
							<p className="text-sm text-on-surface-variant">No data yet. Start tracking!</p>
						</div>
					)}
			...
			<FadeIn delay={0.2}>
				<div className="bg-card rounded-3xl p-5 shadow-[0_12px_32px_rgba(18,28,42,0.04)] border border-outline-variant/10">
					<p className="text-sm font-bold text-foreground mb-3">Daily History</p>
					{weekly.length > 0 ? (
						<div className="space-y-2">
							{[...weekly].reverse().map((day) => {
								...
								const isOnTarget =
									day.totalCalories > 0 && day.totalCalories <= calorieTarget * 1.1;
								const pct =
									calorieTarget > 0 ? Math.round((day.totalCalories / calorieTarget) * 100) : 0;
									...
									<div key={day.date} className="flex items-center gap-3 py-2">
										...
										<div className="text-right shrink-0">
											<p className="text-sm font-bold text-foreground">
												{day.totalCalories > 0 ? `${Math.round(day.totalCalories)} kcal` : "—"}
											</p>
											{day.totalCalories > 0 && (
												<p
